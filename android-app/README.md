...
面向红米 K70 的本地中英双语语音识别 App，当前版本为 `0.10.2`。主界面统一使用本地 HTML、CSS 和 JavaScript，Kotlin 只负责 WebView 宿主、麦克风、识别模型、权限、历史数据库、下载器、通知和系统级悬浮窗等平台能力。识别运行库使用 `sherpa-onnx 1.13.5`。
...
- 双模型方案停止后会按照 Zipformer 检测到的语音端点，使用用户选择的 Paraformer 或 Qwen3-ASR 分段校正；Paraformer 下载小、速度快，Qwen3-ASR 中英文混说质量更高但约 941.3 MiB、处理更慢。若校正结果明显丢句、异常扩张或把英文拆成单字母，会保留更可靠的实时结果。
...
- 系统级悬浮按钮支持拖动、点击开始或停止、长按关闭；网页预览和系统桌面悬浮球统一为白色玻璃质感。待机时显示静止的纯灰圆圈，点击后以 280 毫秒自然变形成蓝青三层声波，并提供轻微大小反馈、开始/结束提示音和紧贴球缘的动态光效。设置中可调整 35%—100% 透明度和 48—88 dp 大小，两处共用同一数值；默认待机不显示文字框，识别时显示，停止后保留最终文字 2 秒，点击文字框可复制并在原位置显示“已复制”1 秒。
...
| 中英双语 Paraformer | 停止后按语音片段校正；单独使用时整段识别 | 81,904,027 字节，约 78.1 MiB |
| Qwen3-ASR 0.6B INT8 | 停止后高质量校正中英文及中英混说 | 987,015,347 字节，约 941.3 MiB |
| 合计 | 三套资源全部安装 | 1,129,062,245 字节，约 1.05 GiB |

录音首页会直接显示当前使用的是一套还是两套模型；应用内“设置 → 识别方案”可选择 Zipformer＋Paraformer 或 Zipformer＋Qwen3-ASR，也可单独使用任一模型；“离线模型”显示每套资源的用途、版本、下载大小、已占空间、进度、速度和预计剩余时间。下载支持暂停、继续、HTTP Range 断点续传、失败重试和 SHA-256 校验；校验成功后才原子写入版本标记。开始下载时还会预留 64 MiB 可用空间。用户可删除或重新下载单个模型。
...
Zipformer、Paraformer 与 Qwen3-ASR 均使用 k2-fsa/sherpa-onnx 官方公开模型，许可证为 Apache-2.0；来源链接记录在资源清单中。
