...
面向红米 K70 的本地中英双语语音识别 App，当前版本为 `0.8.2`。主界面统一使用本地 HTML、CSS 和 JavaScript，Kotlin 只负责 WebView 宿主、麦克风、识别模型、权限、历史数据库、下载器、通知和系统级悬浮窗等平台能力。识别运行库使用 `sherpa-onnx 1.13.5`。
...
- 双模型方案停止后会按照 Zipformer 检测到的语音端点，让 Paraformer 分段校正，降低长录音整段校正产生幻觉的概率；若校正结果明显丢句、异常扩张或把英文拆成单字母，会保留更可靠的实时结果。
...
录音首页会直接显示当前使用的是一套还是两套模型；应用内“设置 → 识别方案”明确列出 Zipformer 与 Paraformer 的组合，“离线模型”显示每套资源的用途、版本、下载大小、已占空间、手机可用空间、进度、速度和预计剩余时间。下载支持暂停、继续、HTTP Range 断点续传、失败重试和 SHA-256 校验；校验成功后才原子写入版本标记。开始下载时还会预留 64 MiB 可用空间。用户可删除或重新下载单个模型。
