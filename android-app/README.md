...
面向红米 K70 的本地中英双语语音识别 App，当前版本为 `0.10.0`。主界面统一使用本地 HTML、CSS 和 JavaScript，Kotlin 只负责 WebView 宿主、麦克风、识别模型、权限、历史数据库、下载器、通知和系统级悬浮窗等平台能力。识别运行库使用 `sherpa-onnx 1.13.5`。
...
- 系统级悬浮按钮支持拖动、点击开始或停止、长按关闭；采用透明玻璃质感、边缘微光和多层连续曲线表示真实收音。设置中可调整 35%—100% 透明度和 48—88 dp 大小，网页预览与系统桌面悬浮球共用同一数值，并可独立关闭悬浮识别文字框。
