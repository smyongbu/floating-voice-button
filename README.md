...
- Windows 正式设置页现有 3 个本地识别选项：`Faster-Whisper Small`、`Qwen3-ASR 0.6B INT8` 和 `Qwen3-ASR 1.7B`，另有 `Streaming Paraformer` 与 `Zipformer` 两个实时模型。五个模型缺失时都会在卡片上显示下载按钮；多文件模型按总字节数汇总进度，并支持暂停、继续、断点续传、失败重试及逐文件 SHA-256 校验。
