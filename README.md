...
- 停止录音后会立即把音频加入后台识别队列，可以继续录制下一段，不必等待上一段完成；同一批录音全部识别后只向剪贴板和原软件输出一次，正文按录制顺序分段排列。数字静音会快速结束，不再触发耗时的最终模型加载。
- “识别历史”会显示排队、识别中和完成状态；顶部只显示排队加识别中的待处理总数，当前识别内容会逐字出现，排队项不显示占位正文。历史会在本机保存最近 500 条记录，并支持搜索、查看全文、复制、删除和清空；复制全部只包含已完成文字，按从新到旧顺序用空行分隔。
...
- Windows 正式设置页现有 3 个本地识别选项：`Faster-Whisper Small`、`Qwen3-ASR 0.6B INT8` 和 `Qwen3-ASR 1.7B`，另有 `Streaming Paraformer` 与 `Zipformer` 两个实时模型。五个模型缺失时都会在卡片上显示下载按钮；多文件模型按总字节数汇总进度，并支持暂停、继续、取消、断点续传、失败重试及逐文件 SHA-256 校验。
