...
- 当前源码版本为 `0.12.2`，识别运行库为 `sherpa-onnx 1.13.5`，详细构建、模型同步和日志说明见 `android-app/README.md`。
...
- 停止录音后会立即把音频加入后台识别队列，可以继续录制下一段，不必等待上一段完成；同一批录音全部识别后按来源软件窗口分组，每个目标窗口只输出一次，正文按录制顺序分段排列，避免切换软件后把文字粘贴到错误窗口。队列最多保留 32 段或 256 MiB 音频；数字静音、稳定底噪和孤立脉冲会快速结束，不再触发耗时的最终模型加载。
- “识别历史”会显示排队、识别中和完成状态；顶部只显示排队加识别中的待处理总数，当前识别内容会随模型结果出现，排队项不显示占位正文。历史会在本机保存最近 500 条记录，并支持搜索、查看全文、复制、删除和清空；复制全部只包含已完成文字，按从新到旧顺序用空行分隔。程序异常退出后，遗留的排队或识别中记录会在下次启动时明确标记为失败，不会永久卡在待处理状态。
...
- Windows 正式设置页现有 3 个本地识别选项：`Faster-Whisper Small`、`Qwen3-ASR 0.6B INT8` 和 `Qwen3-ASR 1.7B`，另有 `Streaming Paraformer` 与 `Zipformer` 两个实时模型。五个模型缺失时都会在卡片上显示下载按钮；多文件模型按总字节数汇总进度，并支持暂停、继续、取消、断点续传、失败重试及逐文件 SHA-256 校验。取消下载只清理对应 `.part` 临时文件，绝不删除已有完整模型或可信校验凭据；同一目标有跨进程互斥保护，并把同时工作的资源文件限制为 3 个。
...
- Windows 所有现用模型统一以 `O:\程序\共享模型仓库` 为规范源；源码通过相对于共同上级目录的路径定位，也可用 `VOICE_INPUT_MODEL_REPOSITORY` 覆盖。源码运行时直接读取共享仓库，不再复制或识别 `%LOCALAPPDATA%\FloatingVoiceButton\models` 中的旧开发缓存；共享仓库不可用时会明确显示模型不可用。编译后的正式 EXE 仍使用 `%LOCALAPPDATA%\FloatingVoiceButton\release-models-v1`，不会读取旧开发缓存；删除任何消费缓存都不会删除共享仓库。
