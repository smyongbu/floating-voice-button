...
- 只要录音过程中已经显示过有效实时文字，这份文字就会作为有效语音证据保留；即使收尾解码为空或音量较低，也不会再误报“没有识别到有效语音”。真正没有文字的数字静音仍会快速结束。
...
- 停止收音后会立即进入后台最终识别队列，可继续录制下一段；首页和记录页会显示当前处理与等待数量。
- 安装后的应用名称统一为“语点”；当前源码版本为 `0.13.0`，识别运行库为 `sherpa-onnx 1.13.5`，详细构建、模型同步和日志说明见 `android-app/README.md`。
...
- 停止录音后会立即把音频加入后台识别队列，可以继续录制下一段，不必等待上一段完成；同一批录音全部识别后按来源软件窗口分组，每个目标窗口只输出一次，正文按录制顺序分段排列，避免切换软件后把文字粘贴到错误窗口。队列最多保留 32 段或 256 MiB 音频；没有实时文字的数字静音、稳定底噪和孤立脉冲会快速结束，不再触发耗时的最终模型加载。
- “识别历史”会显示排队和识别中状态，完成项直接显示正文与时间，不再重复显示“已完成”；顶部只显示排队加识别中的待处理总数，当前识别内容会随模型结果出现，排队项不显示占位正文。列表时间包含两位年份，完成项详情不再重复显示日期时间。历史会在本机保存最近 500 条记录，并支持搜索、查看全文、复制、删除和清空；复制全部只包含已完成文字，按从新到旧顺序用空行分隔。程序异常退出后，遗留的排队或识别中记录会在下次启动时明确标记为失败，不会永久卡在待处理状态。
...
- Windows Release 同时提供两种：轻量版不带模型，适合升级并复用已有模型；五模型完整版附带 Streaming Paraformer、Zipformer、Faster-Whisper Small、Qwen3-ASR 0.6B INT8 与 Qwen3-ASR 1.7B Q5_K_M。由于 GitHub 单个附件限制，完整版采用分卷 7z，需下载全部分卷后用 7-Zip 打开 `.7z.001`。
